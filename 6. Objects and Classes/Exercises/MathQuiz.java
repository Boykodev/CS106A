			...
			if (isPlus()) {
				if ((n1 + n2) <= MAX_NUMBER) {
					for (int j = 0; j < TRIALS; j++) {
						if (j == 0) {
							answer = readInt("What is " + n1 + " + " + n2 + "? ");
						} else {
							answer = readInt(incorrect());
						}
						if (check(answer, n1 + n2)) {
							println(correct());
							break;
						}
						if (j == TRIALS - 1) println("No, the answer is " + (n1 + n2) + ".");
					}
					i++;
				}
			} else {
				if (Math.max(n1, n2) - Math.min(n1, n2) > 0 &&
					...
					for (int j = 0; j < TRIALS; j++) {
						if (j == 0) {
							answer = readInt("What is " + Math.max(n1, n2) + " - " + Math.min(n1, n2) + "? ");
						} else {
							answer = readInt(incorrect());
						}
						...
						if (j == TRIALS - 1) {
							println("No, the answer is " + (Math.max(n1, n2) - Math.min(n1, n2)) + ".");
						}
					}
					i++;
				}
