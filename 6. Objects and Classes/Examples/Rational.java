...
/** Creates a new Rational initialized to zero. */
   public Rational() { this(BigInteger.ZERO); }
   
/**
 * Creates a new Rational from the integer argument.
 * @param n The initial value
 */
   public Rational(BigInteger n) { this(n, BigInteger.ONE); }
 ...
 */
   public Rational(BigInteger x, BigInteger y) {
      ...
      if (y.equals(0)) throw new ErrorException("Division by 0");
      BigInteger g = gcd(x.abs(), y.abs());
      num = x.divide(g);
      den = y.abs().divide(g);
      if (y.compareTo(BigInteger.ZERO) == -1) num.negate();
      }
 ...
 */
   ...
   * @param bigInteger First integer
   * @param bigInteger2 Second integer
